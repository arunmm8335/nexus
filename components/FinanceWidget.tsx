...
export const FinanceWidget: React.FC<FinanceWidgetProps> = ({ onFinanceUpdate }) => {
  ...
  return (
    <GlassCard 
      ...
      action={
        <button onClick={() => navigate('/finance')} className="hover:text-white transition-colors flex items-center gap-1 text-xs uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="mb-6 relative group flex-shrink-0">
          <div className="flex justify-between text-xs mb-2 uppercase tracking-wider font-semibold" style={{ color: 'var(--text-muted)' }}>
            <span>Utilization</span>
            <span>₹{totalSpent.toFixed(0)} <span style={{ color: 'var(--text-main)' }}>/</span> ₹{budget}</span>
          </div>
          <div className="h-3 rounded-full overflow-hidden relative" style={{ backgroundColor: 'var(--input-bg)' }}>
            <div 
          ...
          <div className={`text-right mt-2 text-base font-mono font-medium ${remaining >= 0 ? 'text-[var(--text-main)]' : 'text-red-400'}`}>
            ...
            <div className="flex flex-col h-full gap-4">
                    ...
                    <div className="h-40 w-full flex-shrink-0 relative overflow-hidden">
                       <ResponsiveContainer width="99%" height="100%">
                          <PieChart>
                             ...
                             <RechartsTooltip 
                                contentStyle={{ backgroundColor: 'var(--glass-bg)', borderColor: 'var(--glass-border)', borderRadius: '8px', color: 'var(--text-main)', fontSize: '12px' }}
                       ...
                       <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Spent</span>
                       </div>
                    </div>
                 ) : (
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-2 min-h-0 custom-scrollbar">
                  {transactions.slice(0, 5).map(t => (
                    <div key={t.id} className="flex justify-between items-center text-sm p-3 rounded-lg transition-colors border border-transparent group shrink-0" style={{ backgroundColor: 'var(--input-bg)' }}>
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-full group-hover:bg-sky-500/10 transition-colors" style={{ backgroundColor: 'rgba(0,0,0,0.2)' }}>
                          <IndianRupee className="w-4 h-4 transition-colors" style={{ color: 'var(--text-muted)' }} />
                        </div>
                        <div className="flex flex-col">
                            <span className="font-medium" style={{ color: 'var(--text-main)' }}>{t.category}</span>
                            <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{new Date(t.date).toLocaleDateString()}</span>
                        </div>
                      </div>
                      <span className="font-mono font-bold tracking-tight" style={{ color: 'var(--text-main)' }}>-₹{t.amount}</span>
                    </div>
                  ))}
